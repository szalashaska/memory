...
if DB_TYPE == "postgres":
    ...
    class Scores(db.Model):
        ...
        timestamp = db.Column(
            db.DateTime, default=datetime.now().strftime("%x %X"), nullable=False
        )
...
def register():
    ...
    if request.method == "POST":
        ...
        # If we use Sqlite
            ...
            db.execute(
                "INSERT INTO users (username, hash, color) VALUES (?, ?, ?)",
                (username, hash_password, color),
            )
...
def getscores():
    ...
    # If we use Sqlite
        ...
        db = memory.cursor()
        db.execute(
            """INSERT INTO scores (score, user_id, timestamp) VALUES (?, (SELECT id FROM users WHERE username = ?), 
...
def deletephoto():
    ...
    if DB_TYPE == "postgres":
        ...
        photo = (
            ...
            .filter(Images.image == image, Images.user_id == user.id)
            .first()
        )
    ...
    # If we use Sqlite
        ...
        rows = db.execute(
            """SELECT * FROM images WHERE user_id = (SELECT id FROM users WHERE username = ?) AND image = ?;""",
            (username, image),
        )
        ...
        # Delete the photo
        db.execute(
            """DELETE FROM images WHERE user_id = (SELECT id FROM users WHERE username = ?) AND image = ?;""",
            (username, image),
        )
...
def getlikes():
    ...
    # If we use "sqlite"
        ...
        db.execute(
            """INSERT INTO images (image, url, author, user_id) VALUES 
                (?, ?, ?, (SELECT id FROM users WHERE username = ?));""",
            (image, url, author, username),
        )
...
def scores():
    ...
    if DB_TYPE == "postgres":
        ...
        top_scores = (
            db.session.query(Scores, Users)
            .join(Users)
            .with_entities(Scores.score, Users.username)
    ...
    # If we use Sqlite
        ...
        top_scores = db.execute(
            """ SELECT users.username, scores.score FROM scores JOIN users
...
def myaccount():
    ...
    if DB_TYPE == "postgres":
        ...
        scores = (
            ...
            .with_entities(Scores.score, Scores.timestamp)
            .order_by(Scores.score.desc())
            .limit(10)
            .all()
        )
        ...
        # Get images
        photos = (
            db.session.query(Images, Users)
            .join(Users)
            .where(Users.username == username)
            .with_entities(Images.image, Images.url, Images.author)
            .all()
        )
    ...
    # If we use Sqlite
        ...
        # Get scores from db
        your_score = db.execute(
            """ SELECT scores.score, scores.timestamp FROM scores JOIN users ON scores.user_id = users.id
            WHERE users.username = ? ORDER BY scores.score DESC LIMIT 10;""",
            [username],
        )
        ...
        your_photos = db.execute(
            """ SELECT images.image, images.url, images.author FROM images JOIN users ON images.user_id = users.id
    ...
    return render_template(
        "myaccount.html", username=username, your_score=scores, your_photos=photos
    )
...
def game():
    ...
    if request.method == "POST":
        ...
        if not cards_quantity or not game_theme:
            return sorry(
                "You did not provide theme or number of cards. Mayby next time try a Quick Start!"
            )
